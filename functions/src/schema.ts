// Mirrors the real "ACORD Data Sheet Template" (ACORD 125/126/130/823-825
// Cyber/140, plus a Statement of Values schedule) that agents fill out for a
// submission. Field keys/labels/order must be kept in sync with
// web/src/config/hotelSchema.ts, since the two projects don't share a
// package.
//
// Types are deliberately "string" almost everywhere: real filled-in values
// routinely carry annotations ("$3,000,000 ($2,000,000 hotel / $1,000,000
// restaurant)", "4/10", "1985 (renovated 2015)") that a numeric coercion
// would mangle. "number" is only used for the SOV schedule's genuinely
...
function field(section: string, label: string, key: string, description?: string): FieldDef {
  return { section, label, key, type: "string", description: description ?? label };
}

export const PROFILE_FIELDS: FieldDef[] = [
  // ACORD 125 — Producer
  field("Producer", "Producer", "producer", "Producer/agency name"),
  field("Producer", "Contact", "producerContact", "Producer contact name, email, phone"),
  ...
  field("Applicant Information", "First Named Insured", "firstNamedInsured", "Legal name of the primary insured entity"),
  field("Applicant Information", "DBA", "dba", "Doing-business-as / trade name"),
  ...
  field("Applicant Information", "Entity Type", "entityType", "e.g. LLC, Corporation, Partnership"),
...
// Statement of Values — one row per building/location. The real template
// has no fixed row count ("Add rows as needed"), so the parser reads every
// row up to a TOTAL marker or the end of the sheet, not a hardcoded limit.
export const LOCATION_FIELDS: FieldDef[] = [
  { section: "SOV", key: "locationNumber", label: "Loc #", type: "string", description: "Location/building number" },
  { section: "SOV", key: "address", label: "Address", type: "string", description: "Address of this location" },
  { section: "SOV", key: "occupancy", label: "Occupancy", type: "string", description: "Occupancy/use of this location" },
  { section: "SOV", key: "constructionType", label: "Construction", type: "string", description: "Construction type" },
  { section: "SOV", key: "yearBuilt", label: "Year Built", type: "number", description: "Year built" },
  { section: "SOV", key: "numberOfStories", label: "# Stories", type: "number", description: "Number of stories" },
  { section: "SOV", key: "squareFootage", label: "Total Sq Ft", type: "number", description: "Total square footage" },
  { section: "SOV", key: "protectionClass", label: "Protection Class", type: "number", description: "ISO protection class" },
  { section: "SOV", key: "sprinklered", label: "Sprinklered?", type: "string", description: "Whether this location is sprinklered" },
  { section: "SOV", key: "buildingValue", label: "Building Value", type: "number", description: "Insured building value" },
  { section: "SOV", key: "bppValue", label: "BPP Value", type: "number", description: "Business personal property value" },
  { section: "SOV", key: "businessIncomeValue", label: "BI/EE Value", type: "number", description: "Business income / extra expense value" },
  { section: "SOV", key: "tiv", label: "TIV", type: "number", description: "Total insured value" },
  { section: "SOV", key: "distanceToCoast", label: "Distance to Coast", type: "string", description: "Distance to coast" },
  { section: "SOV", key: "roofTypeAge", label: "Roof Type/Age", type: "string", description: "Roof type and age" },
  { section: "SOV", key: "floodZone", label: "Flood Zone", type: "string", description: "Flood zone designation" },
];
